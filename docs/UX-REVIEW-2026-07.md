...
`TWOAGREE-MARK.md` v5.1 defines the mark as **three** paths — two leaning strokes **plus the floating gold bar** ("the shared element", the point of the symbol) — L-segments only, and §6 explicitly bans the old coordinates and C-curves. `Mark.tsx` contains exactly those banned coordinates, uses C-curves, and renders **two** paths: no bar anywhere in the UI. `public/favicon.svg` draws the strokes *in gold* on claret — violating RULE 3 (gold marks the bar only, never a stroke). BRAND-REPORT.md records these checks as passing, so the brand-pack integration silently regressed the locked spec — the spec's own rule: "this document wins and the file is wrong." **The couple's symbol of agreement currently has no agreement in it.** And it extends past the app: `public/icon-192.png` — the installed home-screen icon, the most permanent brand surface a couple sees — renders the two strokes in gold with no bar, and the favicon, apple-touch-icon, maskable icons and OG image ship from the same set, so the pinned tab, the installed app, and every shared link wear superseded artwork. Rebuild `Mark.tsx` from §4.1, regenerate the whole icon family + OG image, re-run the §6 greps — or, if the two-stroke cut is now a deliberate choice, write that decision into the spec. `Mark.tsx:7–14`, `public/icon-192.png`, `index.html:7–12`, `TWOAGREE-MARK.md:94, 108–114, 120, 350–351`
